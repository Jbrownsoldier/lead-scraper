    ...
    def enrich_lead(self, lead: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            with DDGS() as ddgs:
                ...
                # We can also do a targeted linkedin search for the owner
                ...
                for r in results:
                    ...
                    for e in found_emails:
                        # Simple filter to avoid false positive image file endings
                    ...
                    if any(sp in href for sp in self.social_platforms):
                        # Filter out basic homepages of the networks
                        if len(href.split('/')) > 3: 
                            socials.add(href)
                    ...
                    if 'linkedin.com' in href and ('owner' in title.lower() or 'ceo' in title.lower() or 'founder' in title.lower()):
                        ...
                        if len(parts) > 1:
                            ...
                            # Ensure it's not grabbing the business name dynamically
        ...
        # Make sure if Google returned a social URL as their 'website', we keep it
        if lead.get("website_status") == "Social-Only" and lead.get("website"):
            socials.add(lead["website"])
